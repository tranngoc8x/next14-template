...
import { formatDateToSQLDateTime } from "libs/dateTimeFormatter";
...
const OrderCard: FC<OrderCardProps> = ({ order }) => {
  return (
    <div className="w-max h-max rounded-lg flex flex-col gap-5 shadow-lg p-6 font-medium">
      <div>
        <h1 className="text-primaryColor font-bold text-base">
          {formatDateToSQLDateTime(order.date)}
        </h1>
      </div>
      ...
      <div>
        <h1 className="text-black text-base">
          Total price:{" "}
          <span className="text-black text-base font-bold">${order.price}</span>
        </h1>
      </div>
    </div>
  );
